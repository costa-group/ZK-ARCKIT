...
def get_new_rep_prod(A, B, translation, p):
    aux = {}
    for (signal_a, value_a) in A.items():
        for (signal_b, value_b) in B.items():
            ...
            prod = modular_operations.multiplyP(value_a, value_b, p)
            if (min_ab, max_ab) in aux:
                aux[(min_ab, max_ab)] = modular_operations.addP(aux[(min_ab, max_ab)], prod, p)
            else:
                aux[(min_ab, max_ab)] = prod
    new_rep = []
    for (values, coef) in aux.items():
        if values[0] == values[1]:
            if values[0] == 0:
                new_rep.append([number_to_small_rep(coef, p)])
            else:
                new_rep.append([number_to_small_rep(coef, p), [translation[values[0]], "2"]])
        else:
            if values[0] == 0:
                new_rep.append([number_to_small_rep(coef, p), [translation[values[1]], "1"]])
            else:
                new_rep.append([number_to_small_rep(coef, p), [translation[values[0]], "1"], [translation[values[1]], "1"]])
    return new_rep

def number_to_small_rep(num, p):
    if modular_operations.isNegativeP(num, p):
        return "-" + str(modular_operations.negateP(num, p))
    else:
        return str(num)
...
class Constraint :
    ...
    def get_new_rep(self, translation):
        new_rep = get_new_rep_prod(self.A, self.B, translation, self.p)
        for (signal, value) in self.C.items():
            coef = modular_operations.negateP(value, self.p)
            if signal != 0:
                new_rep.append([number_to_small_rep(coef, self.p), [translation[signal], "1"]])
            else:
                new_rep.append([number_to_small_rep(coef, self.p)])
        return new_rep

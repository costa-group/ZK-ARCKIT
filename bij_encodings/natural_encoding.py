...
class NaturalEncoder(Encoder):
    ...
    def encode(
        ...
        for class_ in classes[in_pair[0][0]].keys():
            ...
            Options = [
                [signal_options(left_normed[i], right_normed[j][k], mapp) for k in range(len(right_normed[j]))]
            ...
            def extend(formula, info):
                ...
                i_, j_ = ind // len(classes[name1][class_]), ind % len(classes[name1][class_])
                    ...
                    options for options in opset
                        if all(
                            [
                                ...
                                for name, _ in in_pair for signal in options[name].keys() 
                            ]
                        )
                ]
                ...
                # has (signal, {options}) pairs for each signal (and name)
                Product = {
                    name: map(
                    lambda options : options[name].values(),
                    viable_options
                ...
                clauses = list( map(lambda x : x + [-ij], clauses.clauses) )
